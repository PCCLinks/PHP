...
function optionLookup($value, $area, $connection){
    ...
    $result = mysql_query($SQLoption,  $connection) or die("$SQLoption<br/>There were problems connecting to the contact data.  If you continue to have problems please contact us.<br/>");
...
function current_enrolled($keyResourceSpecialistID){
    ...
    if($temporary == "testing") $tmp = "";
     $tmpCreateSQLCase="CREATE ". $tmp ."TABLE IF NOT EXISTS tmpCaseCurrentEnrolled(
     `contactID` int( 11  )  NOT  NULL,
     `firstName` varchar( 50  )  NULL,
     `lastName`  varchar( 50  )  NULL ,
     `bannerGNumber` char( 10  )  DEFAULT NULL ,
     PRIMARY  KEY (  `contactID`  )
    )
    ENGINE  =  MyISAM  DEFAULT CHARSET  = latin1";
    $tmpCreate = mysql_query($tmpCreateSQLCase,  $connection) or die($tmpCreateSQLCase. "<br/>There were problems with create tmp table.<br/>");

  //  $emptySQLCase = "TRUNCATE TABLE `tmpReportCase`";
//    $tmpEmpty = mysql_query($emptySQLCase,  $connection) or die($emptySQLCase. "<br/>There were problems with truncating tmp table<br/>");

    $tmpInsertSQLCase = "INSERT INTO tmpCaseCurrentEnrolled  
        select distinct a.contactID, a.firstName, a.lastName, a.bannerGNumber from contact a 
	   right JOIN status b
    ...
    $result = mysql_query($tmpInsertSQLCase,  $connection) or die("There were problems connecting to the current_enrolled data via contact.  If you continue to have problems please contact us.<br/>");
...
function current_applicant($program){
	...
	join (
	    ...
	    group by x.contactID, x.undoneStatusID 
           HAVING x.program = '$program' AND x.undoneStatusID IS NULL AND x.keyStatusID in (1,2,4,5,8)
	    )
...
function currentEnrolledRS($keyResourceSpecialistID, $history){
   ...
   //    $sql_cache = "RESET QUERY CACHE";
   //     $result = mysql_query($sql_cache,  $connection) or die("There were problems resetting cache.  If you continue to have problems // please contact us.<br/>".$sql_cache);
...
//        $result = mysql_query($sql_cache,  $connection) or die("There were problems resetting cache.  If you continue to have problems //please contact us.<br/>".$sql_cache);
       ...
       $SQL = "SELECT distinct a.contactID, a.firstName, a.lastName, a.bannerGNumber   
          FROM contact a 
           RIGHT JOIN status b 
            ON a.contactID=b.contactID  
                AND b.statusID in (SELECT max(statusID) FROM status   
                GROUP BY contactID HAVING (b.undoneStatusID IS NULL) )
                AND b.statusDate in (SELECT max(statusDate) FROM status   
                GROUP BY contactID HAVING (b.undoneStatusID IS NULL) )
                 AND (b.keyStatusID IN (6,7,8,10,11))                
                  RIGHT JOIN statusResourceSpecialist c 
	               ON b.statusID=c.statusID
                    RIGHT JOIN keyResourceSpecialist d 
   	 ...
   	 $SQL = "select distinct a.contactID, a.firstName, a.lastName, a.bannerGNumber from tmpCaseCurrentEnrolled a 
                   right JOIN status b
   	    	      on a.contactID=b.contactID  
  	              RIGHT JOIN status bc 
   	               ON b.statusID=bc.statusID  
                      AND bc.statusID in 
   		          ( 
            			select max(statusID) from status 
   		        group by contactID, undoneStatusID 
   		            HAVING undoneStatusID IS NULL AND keyStatusID in (6,7,8,10,11)
   	           ) 
       		  AND bc.undoneStatusID IS NULL AND bc.keyStatusID in (6,7,8,10,11)   
                       ORDER BY a.lastName, a.firstName";
       }
