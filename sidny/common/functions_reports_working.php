    ...
    $tmpEmptyEnrolled = mysql_query($emptySQLEnrolled,  $connection) or die($emptySQLEnrolled. "<br/>There were problems with truncating tmp1.<br/>");
        ...
        $tmpInsertSQLEnrolled = "INSERT INTO tmpReportEnrolled    
              SELECT d.contactID, d.program, d.statusID AS statusIDEnrolled, d.undoneStatusID AS 
                ...
                FROM
                  (SELECT a.contactID, a.statusID, max(a.statusDate) as max_timestamp, a.statusNotes, 
                     a.keyStatusID, a.program, a.undoneStatusID, a.statusRecordLast
                     FROM status a GROUP BY a.contactID          
                       HAVING a.program='".$searchProgram."' AND a.undoneStatusID IS NULL
                         AND (a.keyStatusID in (2,3,6,7,8,10,11,12)) ";
	 if(!empty($searchStartDate)){
            $tmpInsertSQLEnrolled .= " AND max_timestamp BETWEEN '".$searchStartDate."' AND '".$searchEndDate."'";
	 }
...
$tmpCreateSQLExited="CREATE ". $tmp ."TABLE IF NOT EXISTS tmpReportExited(
    ...
    $tmpEmptyExited = mysql_query($emptySQLExited,  $connection) or die($emptySQLExited. "<br/>There were problems with truncating tmp2.<br/>");
        ...
        FROM
        (SELECT a.contactID, a.statusID, max(a.statusDate) as max_timestamp, a.statusNotes, a.keyStatusID, 
           a.program, a.undoneStatusID, a.statusRecordLast
            FROM status a GROUP BY a.contactID
              HAVING a.program='".$searchProgram."' AND a.undoneStatusID IS NULL AND a.keyStatusID=3 "; 
                  
        if(!empty($searchStartDate)){
            $tmpInsertSQLExited .= " AND max_timestamp BETWEEN '".$searchStartDate."' AND '".$searchEndDate."'";
        }
    ...
    if($temporary == "testing") $tmp = "";
     $tmpCreateSQLApplicant="CREATE ". $tmp ."TABLE IF NOT EXISTS tmpReportApplicant(
            ...
            $tmpEmptyApplicant = mysql_query($emptySQLApplicant,  $connection) or die($emptySQLApplicant. "<br/>There were problems with truncating tmp2.<br/>");
                ...
                FROM
                (SELECT a.contactID, a.statusID, a.statusNotes, a.keyStatusID, a.program, a.undoneStatusID, 
                  a.statusRecordLast, a.statusDate FROM status a WHERE a.statusDate in (SELECT max(statusDate) 
                    FROM status a GROUP BY contactID 
                      HAVING program='".$searchProgram."' AND undoneStatusID IS NULL 
                        AND keyStatusID =1 ) AND  a.program='".$searchProgram."' AND a.undoneStatusID IS NULL 
                        AND a.keyStatusID =1 ";
