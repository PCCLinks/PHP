...
function current_enrolled($keyResourceSpecialistID){
    ...
    $SQL = "select a.contactID, a.firstName, a.lastName, a.bannerGNumber from contact a 
	   right JOIN status b
     ...
     elseif($history == 'current'){   // this is caseload
   	 ...
   	 $SQL = "select a.contactID, a.firstName, a.lastName, a.bannerGNumber from contact a 
	              ...
	              RIGHT JOIN status bc 
	               ON b.statusID=bc.statusID AND b.contactID=bc.contactID AND b.statusDate=bc.statusDate 
                     AND bc.statusDate in 
 		          ( 
            			select max(statusDate) from status 
			        group by contactID, keyStatusID, undoneStatusID 
			            HAVING contactID in ($ids) AND undoneStatusID IS NULL AND keyStatusID in (6)
		           ) 
           		  AND bc.undoneStatusID IS NULL AND bc.contactID in ($ids) AND bc.keyStatusID in (6)
                       where a.contactID in ($ids)      
                       ORDER BY a.lastName, a.firstName";
       }
