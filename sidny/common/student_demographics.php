...
################################################################################################################
    ...
    $race_menuOptions .= "\n<option value=''></option>";
    $SQLrace = "SELECT * FROM keyOption WHERE selectArea = 'race' ORDER BY optionNum" ;
    $result = mysql_query($SQLrace,  $connection) or die("There were problems connecting to the keyOption table.  If you continue to have problems please contact us.<br/>");
    while($row = mysql_fetch_assoc($result)){
...
################################################################################################################
    ...
    $SQLethnicity = "SELECT * FROM keyOption WHERE selectArea = 'ethnicity' ORDER BY optionNum" ;
    $result = mysql_query($SQLethnicity,  $connection) or die("There were problems connecting to the keyOption table.  If you continue to have problems please contact us.<br/>");
    while($row = mysql_fetch_assoc($result)){
    ...
    <div class='contact_left'>  
     <fieldset class='group1'>
		...
		<li>
			...
			<select name='race' id='race' onchange="validateContact(this.form.id,this.name,this[this.selectedIndex].value,'contact', 1)">
			    <?php echo $race_menuOptions; ?>
			</select>
    		</li> 
		<li>
			<label for='ethnicity'>Ethnicity</label>
			<select name='ethnicity' id='ethnicity' onchange="validateContact(this.form.id,this.name,this[this.selectedIndex].value,'contact', 1)">
			    <?php echo $ethnicity_menuOptions; ?>
			</select>
    		</li>
		<li>
			<label for='homeLanguage'>Home Language</label>
			<input type='text' name='homeLanguage' id='homeLanguage' class='textInput' tabindex='6' value='<?php echo $homeLanguage ?>' onchange="validateContact(this.form.id,this.name,this.value,'contact',0)" />
		</li>
		<li>
			<label for='birthPlace'>Birth Place</label>
			<input type='text' name='birthPlace' id='birthPlace' class='textInput' tabindex='7' value='<?php echo $birthPlace ?>' onchange="validateContact(this.form.id,this.name,this.value,'contact', 0)" />
		</li>
           </ol>
          </fieldset>
