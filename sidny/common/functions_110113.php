...
$SQL1 = "SELECT contactID, program, max(statusDate) as MaxStatusDate,  
                SUBSTRING_INDEX(GROUP_CONCAT(keyStatusID ORDER BY statusDate DESC SEPARATOR '-'),'-',1) 
                 AS MostRecentStatusTypeID from status where undoneStatusID is NULL AND 
                  keyStatusID in (2,3,10,11,13,14) 
                  GROUP BY contactID HAVING MostRecentStatusTypeID in (2,10,11,13,14) "; // group by program removed 12/27/12
...
//      $SQL = "SELECT distinct a.contactID, a.firstName, a.lastName, a.bannerGNumber, a.emailPCC, a.emailAlt     
//          FROM contact a 
//           RIGHT JOIN status b 
//            ON a.contactID=b.contactID  
//              AND b.statusDate in 
//               (SELECT dd.maxStatusDate from 
//                (SELECT max(statusDate) as maxStatusDate, undoneStatusID  FROM status    
//                 GROUP BY contactID, keyStatusID HAVING keyStatusID=6 AND (undoneStatusID IS NULL) AND contactID in ($ids) 
//                 ) dd 
//                ) AND (b.undoneStatusID IS NULL) AND b.contactID in ($ids) AND b.keyStatusID=6   
//                  RIGHT JOIN statusResourceSpecialist c 
//	               ON b.statusID=c.statusID
//                    RIGHT JOIN keyResourceSpecialist d 
           ...
           (
             select substring_index(dd.maxDateString, ':', -1) as statusID_val from 
              ...
              SELECT max(concat(f.statusDate, ':',if(f.keyStatusID=3, 59, f.keyStatusID), ':', statusID)) as  maxDateString,  f.undoneStatusID 
               FROM status f 
                   where f.keyStatusID in (1,2,3,4,5,8,10,11,12,13,14) AND 
...
//SELECT d.contactID, d.statusDate, d.keyStatusID, d.program, d.undoneStatusID, d.statusRecordLast, //keyProgram.programName, keyStatus.statusText FROM
//    (select a.contactID, a.statusDate, a.keyStatusID, a.program, a.undoneStatusID, a.statusRecordLast
//	from status a
//	join (
//	    select x.contactID, max(x.statusDate) as max_timestamp, x.keyStatusID, x.undoneStatusID
