...
function current_enrolled(){
	...
	join (
	    ...
	    group by x.contactID, x.undoneStatusID   
           having x.undoneStatusID IS NULL AND x.keyStatusID in (2,3,6,7,10,11,12)
	    )
	b on a.contactID = b.contactID and b.max_timestamp = a.statusDate 
	WHERE a.undoneStatusID IS NULL AND a.keyStatusID in (2,6,7,10,11,12)
    )
    d";
...
// this query does not bring correct values for the rest of the columns corresponding to maximum status date - needs to be corrected 04/05/12
...
function currentEnrolledRS($keyResourceSpecialistID, $history){
    ...
    $ids = join(',',$arrEnrolledList);
        ...
        //The sql for 'all' doesn't filter to only the enrolled students.
    if($history=='All'){
	$SQL = "SELECT contact.contactID, contact.lastName, contact.firstName, contact.bannerGNumber, status.program, keyResourceSpecialist.rsName
	FROM contact
	LEFT JOIN status ON contact.contactID = status.contactID
	...
	WHERE contact.contactID IN ($ids) AND status.keyStatusID=6";
	//if($keyResourceSpecialistID>0){
	//    $SQL .= " AND statusResourceSpecialist.keyResourceSpecialistID='$keyResourceSpecialistID'";
	//}
	...
	(select a.contactID, a.statusDate, a.keyStatusID, a.statusID, a.program, a.undoneStatusID, a.statusRecordLast
		...
		select x.contactID, max(x.statusDate) as max_timestamp, x.keyStatusID, x.undoneStatusID
		from status x 
		group by x.contactID, x.keyStatusID 
              HAVING x.undoneStatusID IS NULL AND x.contactID IN ($ids) AND x.keyStatusID in (6)
		)
	    b on a.contactID = b.contactID and a.statusDate = b.max_timestamp  
	    WHERE a.undoneStatusID IS NULL AND a.contactID IN ($ids) AND a.keyStatusID in (6)
	)
	...
	while($row = mysql_fetch_assoc($result)){
		if($contact1 != $row['contactID']){
		    $arrStudentList[$row['contactID']]= array('contactID' => $row['contactID'], 'bannerGNumber' => $row['bannerGNumber'], 'lastName' => $row['lastName'], 'firstName' => $row['firstName'], 'rsName' => $row['rsName']);
		}else{
...
function displayListRS($keyResourceSpecialistID, $history, $display, $arrStudentList){
	...
	if(substr_count($studentList['rsName'], $resourceSpecialistName)>0){
		...
		$caseLoadList .= "<li><a href='sidny.php?cid=".$studentList['contactID']."'>".$studentList['bannerGNumber']."</a><a href='sidny.php?cid=".$studentList['contactID']."'>".$studentList['lastName'].", ".$studentList['firstName']."</a>".$studentList['rsName'];
	    }
	}
    }
