        ...
        FROM status d 
          RIGHT JOIN tmpReportStatus b ON d.contactID=b.contactID  
               ...
               LEFT JOIN keySchoolDistrict ON keySchoolDistrict.keySchoolDistrictID = statusSchoolDistrict.keySchoolDistrictID 
                WHERE d.undoneStatusID IS NULL AND d.keyStatusID = 7 ";
       // if($searchSchoolDistrictID) $tmpInsertSQLSD .= " AND statusSchoolDistrict.keySchoolDistrictID = ".$searchSchoolDistrictID;
             
      //    $tmpInsertSQLSD .= " GROUP BY d.contactID HAVING d.undoneStatusID IS NULL AND d.keyStatusID = 7 ORDER BY d.contactID, d.statusRecordLast";
...
function tmpReportReason($searchProgram, $searchStatusReasonID, $temporary){
    ...
    `keyReasonID` varchar( 255 ) DEFAULT NULL ,
    ...
    $tmpEmptyReason = mysql_query($emptySQLReason,  $connection) or die($emptySQLReason. "<br/>There were problems with truncating tmp3.<br/>");
        ...
        concat(statusReason.keyStatusReasonID, ":", keyStatusReason.orderNumber) , keyStatusReason.reasonText, statusReason.statusReasonRecordStart AS statusReasonRecordStart, statusReason.statusReasonRecordLast AS statusReasonRecordLast, d.statusRecordLast AS statusRecordLastReason 
        FROM status d 
          RIGHT JOIN tmpReportStatus b ON d.contactID=b.contactID and d.statusID=b.statusID 
        ...
        WHERE d.program='".$searchProgram."' AND d.undoneStatusID IS NULL AND d.keyStatusID = 3 AND (keyStatusReason.orderNumber != 0) ";
        if($searchStatusReasonID){ 
                      $tmpInsertSQLReason .= " AND statusReason.keyStatusReasonID = ".$searchStatusReasonID; 
        }
...
function tmpReportReasonSecondary($searchProgram, $searchStatusReasonSecondaryID, $temporary){
    ...
    `keyReasonSecondaryID` varchar( 255 ) DEFAULT NULL ,
    ...
    $tmpEmptyReasonSecondary = mysql_query($emptySQLReasonSecondary,  $connection) or die($emptySQLReasonSecondary. "<br/>There were problems with truncating tmp3.<br/>");
        ...
        SELECT d.contactID, d.program, d.statusID AS statusIDReasonSecondary, d.undoneStatusID AS undoneStatusIDReasonSecondary, d.statusNotes AS statusNotesReasonSecondary, d.statusDate AS statusDateReasonSecondary, max(concat(statusReasonSecondary.statusReasonSecondaryID, ":", keyStatusReason.orderNumber)) AS statusReasonSecondaryID, 
  ...
  d.statusRecordLast AS statusRecordLastReasonSecondary 
        FROM status d 
          RIGHT JOIN tmpReportStatus b ON d.contactID=b.contactID and d.statusID=b.statusID 
        ...
        WHERE d.program='".$searchProgram."' AND d.undoneStatusID IS NULL AND d.keyStatusID = 3 AND (keyStatusReason.orderNumber != 0) ";
        if($searchStatusReasonSecondaryID){
            $tmpInsertSQLReasonSecondary .= " AND statusReasonSecondary.keyStatusReasonID = ".$searchStatusReasonID;
        }
        $tmpInsertSQLReasonSecondary .= " group by statusReasonSecondary.statusID ORDER BY d.contactID, d.statusRecordLast";
