...
$tmpCreateSQLRS="CREATE ". $tmp ."TABLE IF NOT EXISTS tmpReportRS(
    ...
    $SQLcontactID = "SELECT contactID FROM tmpReportStatus ORDER BY contactID";
      ...
      foreach($contactIDarray as &$contactID){
          $tmpRecentRS = " SELECT a.statusID from status a 
                RIGHT JOIN status b on a.contactID=b.contactID and a.statusID=b.statusID
                  where b.statusID = (select max(statusID) from status group by contactID, program, 
                   keyStatusID having contactID=$contactID and program='".$searchProgram."' and keyStatusID=6) 
                    and b.contactID= $contactID and b.program='".$searchProgram."' and b.keyStatusID=6
              ) WHERE contactID = $contactID";
