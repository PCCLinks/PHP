...
//Create form inputs for secondary exit reasons.
	...
	$SQLexitReasonSecondary = "SELECT * FROM statusReasonSecondary WHERE statusID = '".$statusID."' AND keyStatusReasonID ='". $row['keyStatusReasonID']."'" ;
	$resultReasonSecondary = mysql_query($SQLexitReasonSecondary,  $connection) or die($SQLexitReasonSecondary."There were problems connecting to the status secondary reasons data.  If you continue to have problems please contact us.<br/>");
	$countSecondary = mysql_num_rows($resultReasonSecondary);
	if($countSecondary ==1)$checkedOption = ' checked';
	if($row['ppsRequired']==1){
	    $ppsCount++;
	    $exitReason_checkboxOptionsPPS .= "\n<input".$checkedOption." type='checkbox' id='factor_PPS".$ppsCount."' class='keyStatusReasonID[]' name='keyStatusReasonID[]' value='".$row['keyStatusReasonID']."' onchange=\"ajaxAddEditStatus('keyStatusReasonID',this.value,'statusReasonSecondary', 0)\" >". $row['reasonText']."<br/>";
	}else{
	    $exitReason_checkboxOptions .= "\n<input".$checkedOption." type='checkbox' class='keyStatusReasonID[]' name='keyStatusReasonID[]' value='".$row['keyStatusReasonID']."' onchange=\"ajaxAddEditStatus('keyStatusReasonID',this.value,'statusReasonSecondary', 0)\" >". $row['reasonText']."<br/>";
	}
	$checkedOption = "";
    }
################################################################################################################
