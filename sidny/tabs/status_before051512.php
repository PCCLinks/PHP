...
if(!empty($statusID)){
$SQLstatus = "SELECT status.program, status.statusDate, status.statusNotes, status.keyStatusID, statusReason.keyStatusReasonID, statusReason.statusReasonID, statusResourceSpecialist.keyResourceSpecialistID, statusResourceSpecialist.statusResourceSpecialistID, statusSchoolDistrict.keySchoolDistrictID, statusSchoolDistrict.statusSchoolDistrictID, statusSchoolDistrict.studentDistrictNumber, statusReason.schoolName
FROM status
...
//Create form inputs for secondary exit reasons.
   ...
   // $exitReasonSecondary_radioOptions .= "\n<div class='checkbox_col'>"; 
	...
	$exitReasonSecondary_radioOptions .= "\n<input".$checkedOption." type='radio' class='keyStatusReasonID' name='keyStatusReasonID' value='".$row['keyStatusReasonID']."' onchange=\"ajaxAddEditStatus('keyStatusReasonID',this.value,'statusReasonSecondary')\" >". $row['reasonText']."<br/>";
	$checkedOption = "";
	$selectedOption = "";
	//start next column
...
//    $ppsCount=0;
//    while($row = mysql_fetch_assoc($result)){
//	//Compare keyStatusReasonID from statusReasonSecondary Table to check or uncheck checkbox option.
//	$SQLexitReasonSecondary = "SELECT * FROM statusReasonSecondary WHERE statusID = '".$statusID."' AND keyStatusReasonID ='". $row 
// ['keyStatusReasonID']."'" ;
//	$resultReasonSecondary = mysql_query($SQLexitReasonSecondary,  $connection) or die($SQLexitReasonSecondary."There were problems
// connecting to the status secondary reasons data.  If you continue to have problems please contact us.<br/>");
//	$countSecondary = mysql_num_rows($resultReasonSecondary);
//	if($countSecondary ==1)$checkedOption = ' checked';
//	if($row['ppsRequired']==1){
//	    $ppsCount++;
//	    $exitReason_checkboxOptionsPPS .= "\n<input".$checkedOption." type='checkbox' id='factor_PPS".$ppsCount.
// "' class='keyStatusReasonID[]' name='keyStatusReasonID[]' value='".$row['keyStatusReasonID']."' onchange=\"ajaxAddEditStatus 
// ('keyStatusReasonID',this.value,'statusReasonSecondary', 0)\" >". $row['reasonText']."<br/>";
//	}else{
//	    $exitReason_checkboxOptions .= "\n<input".$checkedOption." type='checkbox' class='keyStatusReasonID[]' name='keyStatusReasonID[]'
// value='".$row['keyStatusReasonID']."' onchange=\"ajaxAddEditStatus('keyStatusReasonID',this.value,'statusReasonSecondary', 0)\" >". $row  
//['reasonText']."<br/>";
//	}
//	$checkedOption = "";
//    }
...
<div id='statusList'>
    ...
    <form id='fStatus' class="cmxform" action='https://pcclamp.pcc.edu/sidny/common/addedit.php' method='post'>
	...
	<fieldset class='group'>
		...
		<div id='statusStep3'>
			...
			<input type='text' name='statusDate' id='statusDate' class='textInput' value='<?php echo $statusDate ?>'tabindex='2' onchange="validateStatus('statusDate',this.value,'status', 1)"/>
		    </li>
		</div>
	    </ol>
	</fieldset>
	     ...
	     <ol class='dataform'>
			<li>
				...
				<div id='exitReasonSecondary'>
					...
					Please select one of the factors.
					<br/>
					<div class='exitReasonListPPS'>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		success: function(response){
		    ...
		    if(json.program =='yes') $("#tabs").tabs("enable", 4); //enable the YES! tab
		    if(json.program =='map') $("#tabs").tabs("enable", 5); //enable the MAP tab
		    if(json.program =='pd') $("#tabs").tabs("enable", 6);  //enable the Project Degree tab
		    if(json.program =='fc') $("#tabs").tabs("enable", 7);  //enable the Future Connect tab
