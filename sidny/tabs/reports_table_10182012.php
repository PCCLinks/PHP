...
}elseif($searchReport == 'exitReasonSummary'){ 
...
if(!empty($errorMsg)){
    ...
    // , tmpReportRS.rsName as RSname, tmpReportSD.schoolDistrict
        ...
        $sql = "SELECT a.exitReason, a.Number_of_Students from 
           (SELECT count(reasonText) as Number_of_Students, reasonText as exitReason, tmpReportStatus.contactID FROM tmpReportReason RIGHT JOIN tmpReportStatus on tmpReportReason.contactID = tmpReportStatus.contactID ";
           if($searchResourceSpecialistID) $sql .= " LEFT JOIN tmpReportRS ON tmpReportReason.contactID = tmpReportRS.contactID ";
           if($searchSchoolDistrictID) $sql .= " RIGHT  JOIN tmpReportSD ON tmpReportSD.contactID = tmpReportStatus.contactID AND tmpReportSD.keySDID = $searchSchoolDistrictID ";
    ...
    $sql = "SELECT contact.contactID AS contactID, contact.firstName, contact.lastName, contact.bannerGNumber, contact.pupilNumberSD AS studentSDID, contact.emailPCC, contact.dob, contact.race, contact.ethnicity,  tmpReportStatus.currentStatus ";
           ...
           $sql .= ",  tmpReportReason.reasonText as exitReason, tmpReportReasonSecondary.reasonSecondaryText as secondaryReason " .$searchExtraFields. "
